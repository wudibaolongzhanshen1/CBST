...
from util.symbol.symbol import relu, dropout, conv, bn, pool, fc, softmax_out
from util.symbol.resnet_v1 import conv_stage as conv_stage_v1
...
def rna_feat(conv1_layers, level_blocks):
    ...
    for i, conv1_layer in enumerate(conv1_layers[1:]):
        ...
        res1 = conv_stage(res1, names,
                          ...
                          stride=1,
                          no_bias=True)
    for i in range(level_blocks[level].num):
    ...
    for i in range(1, level_blocks[level].num):
