...
def parse_split_file_tgt(dataset_tgt, split_tgt, data_root=''):
    split_filename = 'data_list/{}/{}.lst'.format(dataset_tgt, split_tgt)
    image_list = []
    label_gt_list = []
    image_data_list = []
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='gta',
                        help='The source dataset to use, e.g. cityscapes, voc.')
    parser.add_argument('--dataset-tgt', dest='dataset_tgt', default='cityscapes',
    ...
    parser.add_argument('--split-tgt', dest='split_tgt', default='train',
    ...
    parser.add_argument('--data-root', dest='data_root',
                        ...
                        default='D:\\MyDataSet\\GTA5', type=str)
    parser.add_argument('--data-root-tgt', dest='data_root_tgt',
                        help='The root data dir. for target domain',
                        default='D:\\developper\\PyCharmProject\\AdaptSegNet\\data\\Cityscapes', type=str)
    parser.add_argument('--output', default='outputs',
                        help='The output dir.')
    parser.add_argument('--model', default='cityscapes_rna-a1_cls19_s8',
    ...
    parser.add_argument('--weights', default='../models/gta_rna-a1_cls19_s8_ep-0000.params',
    ...
    parser.add_argument('--mine-id-address', dest='mine_id_address',
                        ...
                        default='outputs/stats', type=str)
    #
...
def get_dataset_specs_tgt(args, model_specs):
    ...
    if dataset == 'gta' and dataset_tgt == 'cityscapes':
        sys.path[0] = os.path.join(os.path.abspath(os.path.join(os.getcwd(),'..')),'data')
        ...
        id_2_label_tgt = np.array([trainId2label[_].id for _ in trainId2label if _ not in (-1, 255)]) # 存储id到trainid的映射
        valid_labels_tgt = sorted(set(id_2_label_tgt.ravel())) # trainid有效的id的集合
        ...
        from data.cityscapesscripts.helpers.labels_cityscapes_synthia import id2label as id2label_tgt
        from data.cityscapesscripts.helpers.labels_cityscapes_synthia import trainId2label as trainId2label_tgt
        from data.cityscapesscripts.helpers.labels_synthia import id2label as id2label_src
...
class ScoreUpdater(object):
    ...
    def calc_updates(valid_labels, pred_label, label):
        ...
        conf = [len(class_flags[j].intersection(pred_flags[k])) for j in range(num_classes) for k in
                range(num_classes)]
        pixel = [len(class_flags[j]) for j in range(num_classes)]
    ...
    def update(self, pred_label, label, i, computed=True):
        conf, pixel = ScoreUpdater.calc_updates(self._valid_labels, pred_label, label) # conf: 混淆矩阵(19, 19), pixel: 每个类别的像素个数(19,)
    ...
    def scores(self, i=None, logger=None):
        ...
        scores = confs[range(num_classes), range(num_classes), :].sum(1)
...
def _train_impl(args, model_specs, logger):
    ...
    split_filename = 'data_list/{}/{}.lst'.format(margs.dataset, args.split)
    num_source = 0
    with open(split_filename) as f:
        for item in f.readlines():
...
# MST:
"""
mine_id_priority:预测的像素点个数占比小于args.mine_thresh的类别的索引
mine_id:预测的像素点个数最少的args.mine_id_number个类别的索引
"""
def _val_impl(args, model_specs, logger):
    ...
    # image_list_tgt与image_tgt_list一样，是图像的路径，label_gt_list_tgt是对应的标签路径
    ...
    label_stride = margs.feat_stride # label_stride=8，意思是网络输出尺寸是原图尺寸的8分之一，后面还需要线性插值到原图尺寸
    ...
    for i in range(x_num):
        ...
        if do_forward:
            ...
            if batch is None:
                if dargs.ident_size:
                    input_h = make_divisible(imh, margs.feat_stride) #取个整,将input_h调整到feat_stride的整数倍
                ...
                input_label = 255 * np.ones((1, int(label_h * label_w)), np.single)
            ...
            net_preds = np.zeros((margs.classes, int(pred_h), int(pred_w)), np.single)
            sy = sx = pred_stride // 2 + np.arange(test_steps) * pred_stride
            for ix in range(test_steps):
                for iy in range(test_steps):
                    input_data = np.zeros((1, 3, input_h, input_w), np.single)
                    input_data[0, :, :imh, :imw] = nim[:, int(sy[iy]):int(sy[iy] + imh), int(sx[ix]):int(sx[ix] + imw)]
                    batch.data[0] = mx.nd.array(input_data) # 目标域cityscapes的图像(1,3,928,1856)
                    mod.forward(batch, is_train=False)
                    this_call_preds = mod.get_outputs()[0].asnumpy()[0] # (19, 115, 232)
                    ...
                    net_preds[:, iy:int(iy + pred_h):int(test_steps), ix:int(ix + pred_w):int(test_steps)] = this_call_preds # 网络的输出net_preds: (19, 115, 232)
        interp_preds_np = interp_preds_as(rim.shape[:2], net_preds, int(pred_stride), imh, imw) #插值到cropsize之前的大小 ，interp_preds_np:(19,1024,2048)
        ########################### #save predicted labels and confidence score vectors in target domains
        # no prior prediction with trainIDs
        pred_label_np = interp_preds_np.argmax(0) # (1024,2048)
        # no prior prediction with labelIDs
        if dargs.id_2_label_tgt is not None:
            pred_label_np = dargs.id_2_label_tgt[pred_label_np] #  id_2_label:将trainid与id对应关系存储，id_2_label[trainId] = id,id有31个，trainId有19个现在pred_label_np中的像素的值是pred的id
        ...
        out_path_np = osp.join(save_dir_nplabelIds, '{}.png'.format(sample_name)) # 'outputs\\0\\results\\nplabelIds\\frankfurt_000000_000294_leftImg8bit.png'
        ...
        # with prior
        if args.with_prior == 'True':
            probmap = np.multiply(prior_array,interp_preds_np).astype(np.float32) #加入了空间先验信息后的更准确的概率图
        elif args.with_prior == 'False':
            probmap = interp_preds_np.copy().astype(np.float32) # (19,1024,2048)
        pred_label = probmap.argmax(0) # (1024,2048)，存储的trainid，也就是gta2cityscapes这个任务的id（每个label本身有一个id，那个是全局id）
        probmap_max = np.amax(probmap, axis=0) # (1024,2048)，每个像素点的最大概率
        ############################ save confidence scores of target domain as class-wise vectors，存储到变量array_cls0,array_cls1,...,array_cls18中
        for idx_cls in np.arange(0, num_classes):
            idx_temp = pred_label == idx_cls # (1024,2048)的一个蒙版，只有等于idx_cls的像素点为True
            sname = 'array_cls' + str(idx_cls)
            if not (sname in locals()):
                exec("%s = np.float32(0)" % sname,globals()) # 动态定义变量array_cls0,array_cls1,...,array_cls18
            if idx_temp.any():
                cls_exist_array[0, idx_cls] = 1
                probmap_max_cls_temp = probmap_max[idx_temp].astype(np.float32) #probmap_max_cls_temp:(n,)，n是等于idx_cls的像素点的个数，存储的是归类为idx_cls的像素点的概率
        ...
        out_path_probmap = osp.join(save_dir_probmap, '{}.npy'.format(sample_name)) # 'outputs\\probmap\\frankfurt_000000_000294_leftImg8bit.npy'
        np.save(out_path_probmap, probmap.astype(np.float32))
        # save predictions with spatial priors, if sp exist.
        if args.with_prior == 'True':
            if dargs.id_2_label_tgt is not None:
                pred_label = dargs.id_2_label_tgt[pred_label] # pred_label本来存储的是trainid，现在把它转成id
        ...
        if args.split_tgt in ('train', 'val'):
            ...
            label = np.array(Image.open(label_path), np.uint8) #（1024,2048）
            if args.with_prior == 'True':
                scorer.update(pred_label, label, i)  # pred_label是加入了空间先验信息后的预测结果
            scorer_np.update(pred_label_np, label, i) # pred_label_np是没有加入空间先验信息的预测结果

    # save target training list
    fout = 'data_list/{}/{}_training_gpu{}.lst'.format(args.dataset_tgt,args.split_tgt,args.gpus) # 文件中记录映射关系，每一行是一个目标域样本的路径和预测输出的路径
    fo = open(fout, "w")
    ...
    if args.kc_policy == 'cb':
        for idx_cls in np.arange(0, num_classes):
            tname = 'array_cls' + str(idx_cls) # array_cls0,array_cls1,...,array_cls18中存储的是confidence scores，是数组

            if cls_exist_array[0, idx_cls] == 1:
                sorted_array = sorted(eval(tname), reverse=True)  # reverse=False 表示升序排列，reverse=True 表示降序排列
                len_cls = len(sorted_array)  # array_cls0的长度，也就是预测为idx_cls的像素点的个数
                # exec("%s = sorted(%s,reverse=True)" % (tname, tname),globals()) # reverse=False for ascending losses and reverse=True for descending confidence
                # exec("len_cls = len(%s)" % tname,globals()) # array_cls0的长度，也就是预测为idx_cls的像素点的个数
                cls_size[idx_cls] = len_cls
                len_thresh = int(math.floor(len_cls * init_tgt_port))
                if len_thresh != 0:
                    cls_thresh[idx_cls] = sorted_array[len_thresh-1].copy() #cls_thresh[idx_cls]是idx_cls类的confidence score的阈值
                exec("%s = %d" % (tname, 0.0))

    # threshold for mine_id with priority，cls_size:每个类别的像素点个数，cls_size = 0表示没有预测为这个类别的像素点
    mine_id_priority = np.nonzero(cls_size / np.sum(cls_size) < args.mine_thresh)[0] # 这行代码的作用是找到 cls_size 数组中相对大小小于 args.mine_thresh 的类别的索引，并将这些索引存储在 mine_id_priority 变量中
    # chosen mine_id,mine_id_all存储的是cls_size从小到大排序后的索引
    mine_id_all = np.argsort(cls_size / np.sum(cls_size))
    mine_id = mine_id_all[:args.mine_id_number] # 选取cls_size最小的args.mine_id_number个类别
    print(mine_id)
    ...
    for i in range(x_num):
        ...
        cls_thresh[cls_thresh == 0] = 1.0 # cls_thresh = 0 means there is no prediction in this class，cls_thresh是每个类别的confidence score的阈值，如果该像素值预测的类别的score大于阈值，则说明其可信度较高，可分配伪标签
        ############# pseudo-label assignment  probmap:(19,1024,2048)
        for idx_cls in np.arange(0, num_classes):
            cls_thresh_temp = cls_thresh[idx_cls] # cls_thresh_temp是当前idx_cls类的阈值
            cls_probmap = probmap[idx_cls,:,:] # cls_probmap是当前idx_cls类的概率图
            cls_rw_probmap = np.true_divide(cls_probmap,cls_thresh_temp) # cls_probmap逐元素除以cls_thresh_temp,cls_probmap是这个类的概率图，cls_rw_probmap是概率图除以阈值，大于1的地方说明可以分配伪标签
            rw_probmap[idx_cls,:,:] = cls_rw_probmap.copy()
        # rw_probmap[idx_cls,:,:]是idx_cls类的概率图除以阈值，大于1的地方说明可以分配伪标签
        rw_probmap_max = np.amax(rw_probmap, axis=0)
        pseudo_label = np.argmax(rw_probmap,axis=0) # 某个像素值19个类的分数各自除以对应的阈值，哪个类别超出阈值最多，就定义伪标签为哪个类别
        ...
        rw_pred_label = pseudo_label_labelID.copy() # rw_pred_label是每个像素的伪标签的ID(不是trainId)，且未经过阈值处理（小于阈值则应置为0，id为0的label是void）
        # ignore label assignment, compatible with labelIDs
        pseudo_label_labelID[idx_unconfid] = 0 # pseudo_label_labelID是每个像素的伪标签的ID(不是trainId)，且经过阈值处理，小于阈值的像素的伪标签的ID置为0
        ############# save pseudo-label
        im_to_save_pseudo = Image.fromarray(pseudo_label_labelID.astype(np.uint8)) # 经过阈值处理的伪标签图片（不加调色板）
        im_to_save_pseudocol = im_to_save_pseudo.copy() # 经过阈值处理的伪标签图片（加调色板）
