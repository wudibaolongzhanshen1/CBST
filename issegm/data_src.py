...
def parse_split_file(dataset, split, data_root=''):
    split_filename = 'data_list/{}/{}.lst'.format(dataset, split)
    image_list = []
    label_list = []
...
class FileIter(DataIter):
    ...
    def __init__(self,
        ...
        self._label = list({self._label_name: np.zeros((1, int(crop_h * crop_w / self._label_stride**2)), np.single)}.items())
    ...
    def _read(self, db_inds):
        ...
        output = []
        for _ in range(locs_per_crop * self._crops_per_image):
        ...
        for i,db_ind in enumerate(db_inds):
            ...
            for k in range(self._crops_per_image):
                do_flipping = npr.randint(2) == 0
                for j in range(locs_per_crop):
                    ...
                    rlabel = np.array(rlabel)[int(lsy) : target_crop_size : self._label_stride, int(lsx) : target_crop_size : self._label_stride]
                    # flipping
                    if do_flipping:
                        rim = rim[:, ::-1, :]
                        rlabel = rlabel[:, ::-1]
