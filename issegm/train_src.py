...
def get_dataset_specs(args, model_specs):
    ...
    if dataset == 'cityscapes':
        from data.cityscapesscripts.helpers.labels import id2label, trainId2label
        ...
        sys.path[0] = '../'
        from data.cityscapesscripts.helpers.labels import id2label, trainId2label
        # id2label:将id与label对应关系存储，id2label[id] = label
        # trainId2label:将trainid与label对应关系存储，trainId2label[trainId] = label
        # trainId2label:将trainid与label对应关系存储，trainId2label[trainId] = label
        sys.path[0] = os.getcwd()
        # label_2_id:将id与trainid对应关系存储，label_2_id[id] = trainId
        label_2_id = 255 * np.ones((256,))
        for l in id2label:
            if l in (-1, 255):
                continue
            label_2_id[l] = id2label[l].trainId
        # id_2_label:将trainid与id对应关系存储，id_2_label[trainId] = id
        id_2_label = np.array([trainId2label[_].id for _ in trainId2label if _ not in (-1, 255)])
        valid_labels = sorted(set(id_2_label.ravel())) # id_2_label存储的是id，所以valid_labels存储的是排序后的id
        #
        cmap = np.zeros((256, 3), dtype=np.uint8)
        for i in id2label.keys():
            cmap[i] = id2label[i].color
        ...
        from data.cityscapesscripts.helpers.labels_synthia import id2label, trainId2label
        #
        label_2_id = 255 * np.ones((256,))
        for l in id2label:
            if l in (-1, 255):
...
class ScoreUpdater(object):
    ...
    def calc_updates(valid_labels, pred_label, label):
        ...
        conf = [len(class_flags[j].intersection(pred_flags[k])) for j in range(num_classes) for k in range(num_classes)]
